-- ============================================================
-- RecallIQ — Combined migration (fixed order)
...
-- ── Helper functions (no table deps) ────────────────────────
...
create index if not exists conversations_customer_idx       on public.conversations (customer_id, started_at desc) where deleted_at is null;
...
create index if not exists messages_conversation_idx        on public.messages (conversation_id, occurred_at desc) where deleted_at is null;
...
create index if not exists follow_ups_org_status_idx        on public.ai_follow_ups (organization_id, status, due_at asc) where deleted_at is null;
...
create index if not exists runtime_logs_org_idx             on public.runtime_intelligence_logs (organization_id, created_at desc) where deleted_at is null;
create index if not exists routing_logs_org_idx             on public.model_routing_logs (organization_id, created_at desc) where deleted_at is null;
create index if not exists token_usage_org_idx              on public.token_usage_logs (organization_id, created_at desc) where deleted_at is null;
...
create policy user_preferences_update on public.user_preferences for update
  using (member_id in (select tm.id from public.team_members tm where tm.clerk_user_id = public.current_user_id() and tm.deleted_at is null) or public.is_org_admin(organization_id))
  with check (member_id in (select tm.id from public.team_members tm where tm.clerk_user_id = public.current_user_id() and tm.deleted_at is null) or public.is_org_admin(organization_id));
