...
do $$ begin
  create type public.member_role as enum ('owner', 'admin', 'manager', 'rep', 'analyst', 'member');
...
do $$ begin
  create type public.customer_stage as enum ('lead', 'qualified', 'discovery', 'proposal', 'negotiation', 'closed_won', 'closed_lost');
exception when duplicate_object then null; end $$;

do $$ begin
  create type public.conversation_status as enum ('open', 'pending_follow_up', 'closed', 'archived');
exception when duplicate_object then null; end $$;

do $$ begin
  create type public.message_sender_type as enum ('user', 'customer', 'ai', 'system', 'integration');
exception when duplicate_object then null; end $$;

do $$ begin
  create type public.message_format as enum ('plain_text', 'markdown', 'email_html', 'json');
exception when duplicate_object then null; end $$;

do $$ begin
  create type public.follow_up_status as enum ('draft', 'queued', 'sent', 'completed', 'cancelled');
exception when duplicate_object then null; end $$;

do $$ begin
  create type public.memory_entry_kind as enum ('objection', 'tone', 'pricing', 'stakeholder', 'security', 'timeline', 'preference', 'summary', 'strategy');
exception when duplicate_object then null; end $$;

do $$ begin
  create type public.runtime_log_level as enum ('debug', 'info', 'warn', 'error');
exception when duplicate_object then null; end $$;
...
do $$ begin
  create type public.escalation_status as enum ('open', 'investigating', 'resolved', 'dismissed');
exception when duplicate_object then null; end $$;

do $$ begin
  create type public.budget_scope as enum ('organization', 'workspace', 'member', 'model', 'channel');
exception when duplicate_object then null; end $$;

do $$ begin
  create type public.analytics_granularity as enum ('hour', 'day', 'week', 'month');
exception when duplicate_object then null; end $$;
...
create table if not exists public.budget_limits (
  ...
  updated_at timestamptz not null default now(),
  deleted_at timestamptz,
  unique (organization_id, name, period_start, scope, scope_key)
);
...
create table if not exists public.ai_performance_analytics (
  ...
  updated_at timestamptz not null default now(),
  deleted_at timestamptz,
  unique (organization_id, granularity, bucket_start, model_name, routing_strategy)
);
...
create index if not exists customers_owner_idx on public.customers (organization_id, owner_member_id) where deleted_at is null;
...
create index if not exists messages_org_sender_idx on public.messages (organization_id, sender_type, occurred_at desc) where deleted_at is null;
create index if not exists messages_search_idx on public.messages using gin (to_tsvector('english', coalesce(content, ''))) where deleted_at is null;
create index if not exists follow_ups_org_status_due_idx on public.ai_follow_ups (organization_id, status, due_at asc) where deleted_at is null;
...
create index if not exists runtime_logs_org_created_idx on public.runtime_intelligence_logs (organization_id, created_at desc) where deleted_at is null;
create index if not exists routing_logs_org_created_idx on public.model_routing_logs (organization_id, created_at desc) where deleted_at is null;
...
create index if not exists cost_tracking_org_period_idx on public.cost_tracking_entries (organization_id, period_start desc, period_end desc) where deleted_at is null;
create index if not exists analytics_org_bucket_idx on public.ai_performance_analytics (organization_id, granularity, bucket_start desc) where deleted_at is null;
create index if not exists escalations_org_status_idx on public.escalation_events (organization_id, status, severity, created_at desc) where deleted_at is null;
...
create policy organizations_delete on public.organizations for delete using (public.is_org_admin(id));
...
create policy team_members_delete on public.team_members for delete using (public.is_org_admin(organization_id));
...
begin
  foreach target_table in array array[
    ...
    'budget_limits', 'cost_tracking_entries', 'ai_performance_analytics',
    'escalation_events'
  ] loop
    execute format('drop policy if exists %s_select on public.%I', target_table, target_table);
    execute format('create policy %s_select on public.%I for select using (public.is_org_member(organization_id))', target_table, target_table);
    execute format('drop policy if exists %s_update on public.%I', target_table, target_table);
    execute format('create policy %s_update on public.%I for update using (public.is_org_admin(organization_id)) with check (public.is_org_admin(organization_id))', target_table, target_table);
    execute format('drop policy if exists %s_delete on public.%I', target_table, target_table);
    execute format('create policy %s_delete on public.%I for delete using (public.is_org_admin(organization_id))', target_table, target_table);
  end loop;
end $$;
...
create policy user_preferences_select on public.user_preferences for select using (
  member_id in (
    select tm.id
    from public.team_members tm
    where tm.clerk_user_id = public.current_user_id()
      and tm.deleted_at is null
...
create policy user_preferences_delete on public.user_preferences for delete using (public.is_org_admin(organization_id));

create or replace view public.organization_usage_daily as
select
  organization_id,
...
comment on table public.ai_memory_entries is 'Persistent hindsight memory records for RecallIQ.';
comment on table public.runtime_intelligence_logs is 'Runtime observability events for cascadeflow.';
comment on table public.model_routing_logs is 'Model routing decisions and fallbacks.';
comment on table public.token_usage_logs is 'Token accounting per request for cost control.';
comment on table public.cost_tracking_entries is 'Cost aggregation for budgets and forecasting.';
comment on table public.ai_performance_analytics is 'Aggregated AI performance metrics for dashboards.';
comment on table public.escalation_events is 'Escalation lifecycle records for customer risk management.';
